...
void pack(int input, int start_bit, int end_bit, int *value) {
    int num_bits = end_bit - start_bit + 1;
    int mask = (1 << num_bits) - 1;

    input = input & mask;

    *value = *value | (input << start_bit);
}
...
{
    char instruction[40];
    int n = read(STDIN_FD, instruction, 40) - 1;

    InstData data;
    get_inst_data(instruction, &data);

    int packed = 0;

    if (data.type == R) {
        pack(data.opcode, 0, 6, &packed);
        pack(data.rd, 7, 11, &packed);
        pack(data.funct3, 12, 14, &packed);
        pack(data.rs1, 15, 19, &packed);
        pack(data.rs2, 20, 24, &packed);
        pack(data.funct7, 25, 31, &packed);
    }
    else if (data.type == I) {
        pack(data.opcode, 0, 6, &packed);
        pack(data.rd, 7, 11, &packed);
        pack(data.funct3, 12, 14, &packed);
        pack(data.rs1, 15, 19, &packed);
        pack(data.imm & 0xfff, 20, 31, &packed);
    }
    else if (data.type == S) {
        pack(data.opcode, 0, 6, &packed);
        pack(data.imm & 0x1f, 7, 11, &packed);
        pack(data.funct3, 12, 14, &packed);
        pack(data.rs1, 15, 19, &packed);
        pack(data.rs2, 20, 24, &packed);
        pack((data.imm >> 5) & 0x7f, 25, 31, &packed);
    }
    else if (data.type == B) {
        pack(data.opcode, 0, 6, &packed);
        pack((data.imm >> 11) & 0x1, 7, 7, &packed);
        pack((data.imm >> 1) & 0xf, 8, 11, &packed);
        pack(data.funct3, 12, 14, &packed);
        pack(data.rs1, 15, 19, &packed);
        pack(data.rs2, 20, 24, &packed);
        pack((data.imm >> 5) & 0x3f, 25, 30, &packed);
        pack((data.imm >> 12) & 0x1, 31, 31, &packed);
    }
    else if (data.type == U) {
        pack(data.opcode, 0, 6, &packed);
        pack(data.rd, 7, 11, &packed);
        pack(data.imm & 0xfffff, 12, 31, &packed);
    }
    else if (data.type == J) {
        pack(data.opcode, 0, 6, &packed);
        pack(data.rd, 7, 11, &packed);
        pack((data.imm >> 12) & 0xff, 12, 19, &packed);
        pack((data.imm >> 11) & 0x1, 20, 20, &packed);
        pack((data.imm >> 1) & 0x3ff, 21, 30, &packed);
        pack((data.imm >> 20) & 0x1, 31, 31, &packed);
    }
    else {
        char str[] = "error";
        write(STDOUT_FD, str, 5);
    }
