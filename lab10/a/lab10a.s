...
linked_list_search:
    # a0 has the head node pointer
    # a1 has the value we are searching
    li t0, 0                    # index counter

    1:
        beqz a0, not_found

        lw t1, 0(a0)            # first number
        lw t2, 4(a0)            # second number

        add t1, t1, t2          # sum of the numbers

        beq a1, t1, found

        addi t0, t0, 1          # increments the index counter
        lw a0, 8(a0)            # next node become the current node
        j 1b
    1:

    found:
        mv a0, t0               # returns the index
        ret

    not_found:
        li a0, -1               # returns -1 if not found
        ret
